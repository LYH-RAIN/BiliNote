...
def get_style_format(style):
    style_map = {
        'minimal': '1. **精简信息**: 仅记录最重要的内容，简洁明了。但仍需保留核心案例。',
        'detailed': '''2. **详细记录**: 
   - 包含完整的内容和每个部分的详细讨论
   - ❗❗❗ **所有案例必须完整保留**：每个案例的背景、问题、解决方案、结果都要记录
   - 代码示例、数据对比、实际应用场景必须完整呈现
   - 需要尽可能多的记录视频内容，最好详细的笔记
   - 不要省略任何有价值的信息''',
        'academic': '3. **学术风格**: 适合学术报告，正式且结构化。包含完整的研究案例和数据分析。',
        'xiaohongshu': '''4. **小红书风格**: 
### 擅长使用下面的爆款关键词：
